...
const handleStreamingUploadMiddleware = (req: any, res: any, next: any) => {
  uploadParams.single('fileData')(req, res, (err: any) => {
    if (err) {
...
app.post("/api/events/:id/upload/streaming", handleStreamingUploadMiddleware, async (req, res, next) => {
  try {
    ...
    const isVideoExt = ['.mp4', '.mov', '.webm', '.mkv', '.avi', '.3gp', '.3gpp', '.m4v'].includes(ext);
    const type = (req.file.mimetype.startsWith('video') || isVideoExt) ? 'video' : 'photo';
    ...
    // If uploading a video, re-encode (safely wrapped)
    if (type === 'video' && req.file) {
      try {
        ...
        logger.warn("FFmpeg re-encode skipped/failed, keeping original video: " + vErr);
      }
    }
