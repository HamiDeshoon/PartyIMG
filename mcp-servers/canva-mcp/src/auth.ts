...
export async function exchangeCodeForTokens(code: string, codeVerifier: string): Promise<CanvaTokenData> {
  ...
  const response = await axios.post('https://api.canva.com/v1/oauth/token', params.toString(), {
    headers: {
      ...
      server = http.createServer(async (req, res) => {
        try {
          ...
          if (reqUrl.pathname === '/oauth/callback' || reqUrl.pathname === '/callback') {
            ...
            if (!code || recState !== state) {
              res.writeHead(400, { 'Content-Type': 'text/html' });
              res.end('<h2>Authentication Failed</h2><p>Invalid code or state mismatch.</p>');
              server.close();
              return reject(new Error('State mismatch or missing authorization code.'));
            }
